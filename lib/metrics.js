...
import {
  workingHours, budgetFor, budgetForPoints, enteredCurrentStatus, readySince, workStartedAt,
  severityOf, orderStatuses, STATE_FACTOR, BUDGET_HOURS, THRESHOLDS, WORKDAY, HOURS_PER_DAY,
} from "./sla.js";
...
export function computeMetrics(snapshot) {
  ...
  // ── Cuánto lleva cada ticket trabajándose, contra su presupuesto ──
  // El reloj arranca en In Progress y no se reinicia después. Un ticket en
  // To Do no tiene reloj: puede llevar semanas en la cola sin estar vencido,
  // porque nadie lo agarró todavía. Lo que sí mostramos de To Do es la espera,
  // como dato suelto, sin presupuesto ni semáforo.
  const isActiveStatus = (name) => categoryOf(name, statuses) === "indeterminate";
  ...
  for (const i of sprintIssues) {
    ...
    // Espera en To Do: se cuenta desde que quedó libre de bloqueantes, no
    // desde que se creó. Si todavía lo bloquea otro, ni eso — la demora no
    // es del equipo.
    const ready = cat === "new" ? readySince(i, { blockers, doneAt, sprintStart, enteredAt }) : null;
    const blocked = cat === "new" && ready === null;
    const waiting = cat === "new" && !blocked ? workingHours(ready, now) : null;

    const startedAt = cat === "done" ? null : workStartedAt(i, isActiveStatus);
    const budget = startedAt ? budgetFor(pts, status) : null;
    const elapsed = startedAt ? workingHours(startedAt, now) : null;
    ...
    aging.set(i.key, {
      status, category: cat, since: startedAt ?? ready ?? enteredAt, enteredAt, startedAt, blocked, waiting,
  ...
  const alerts = sprintIssues
    ...
    .map((i) => {
      ...
      return {
        ...
        severity: a.severity, overBy: a.overBy, startedAt: a.startedAt,
      };
    })
  ...
  const byStatus = flowOrder.map((name) => {
    ...
    // En To Do no hay reloj, así que la mediana de esa columna es de espera.
    const ages = list.map((i) => aging.get(i.key).elapsed ?? aging.get(i.key).waiting)
  ...
  const table = sprintIssues.map((i) => ({
    ...
    waiting: aging.get(i.key)?.waiting ?? null,
    startedAt: aging.get(i.key)?.startedAt ?? null,
  ...
  return {
    ...
    byStatus,
    alerts,
    sla: { budgets: BUDGET_HOURS, factors: STATE_FACTOR, workday: WORKDAY, hoursPerDay: HOURS_PER_DAY, thresholds: THRESHOLDS },
